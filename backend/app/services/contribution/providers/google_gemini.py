...
def _parse_json_content(content: str) -> dict[str, object]:
    ...
    raise HTTPException(status_code=502, detail="Google Gemini response is not valid JSON.")


class GoogleGeminiContributionProvider:
    def __init__(
        self,
        *,
        api_key: str | None,
    ...
    def evaluate(
        ...
        *,
        ...
        try:
            for chunk in client.models.generate_content_stream(
                ...
                if first_content_seconds is None:
                    ...
                    logger.info(
                        "Google Gemini API 첫 content 청크를 수신했습니다. model=%s first_content_seconds=%.2f",
                        model,
                        first_content_seconds,
                    )
                if now - last_progress_at >= 10:
                    logger.info(
                        "Google Gemini API 스트리밍 응답 생성 중입니다. model=%s elapsed_seconds=%.2f chunks=%s content_chars=%s",
                        model,
                        now - started_at,
                        chunk_count,
            ...
            logger.exception(
                "Google Gemini API 응답 대기 중 오류가 발생했습니다. model=%s elapsed_seconds=%.2f",
                model,
                elapsed_seconds,
            )
        ...
        elapsed_seconds = time.perf_counter() - started_at
        logger.info(
            "Google Gemini API 스트리밍 응답을 수신했습니다. model=%s elapsed_seconds=%.2f chunks=%s content_chars=%s first_content_seconds=%s",
            model,
            elapsed_seconds,
            chunk_count,
            sum(len(part) for part in content_parts),
        ...
        if not content.strip():
            raise HTTPException(status_code=502, detail="Google Gemini returned an empty response.")
